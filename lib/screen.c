...
screen_initrender(struct render *r){
	R = r;
	screen_init(SCREEN.wifth,SCREEN.height,SCREEN.scale);
}
...
void screen_init(float w,float h,float scale){
	...
	SCREEN.height = (int)h;
	SCREEN.scale = scale;
	SCREEN.invw = 2.0 / SCREEN_SCALE / w;
	SCREEN.invh = -2.0 / SCREEN_SCALE / h;
	if(R){
...
bool
screen_is_poly_invisible(const float *points,int len,int stride){
	int i = 0;
	bool invisible = true;
